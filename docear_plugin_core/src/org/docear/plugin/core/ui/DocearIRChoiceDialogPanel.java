...
public class DocearIRChoiceDialogPanel extends JPanel {
	...
	public DocearIRChoiceDialogPanel() {
		...
		headPanel.add(lblDescriptionhead, "2, 2");
		...
		headPanel.add(separator_6, "1, 6, 3, 1");
		...
		userDataPane.add(lblUsername, "2, 2");
		
		lblEmail = new JLabel("eMail");
		userDataPane.add(lblEmail, "4, 2");
		
		txtUsername = new JTextField();
		userDataPane.add(txtUsername, "2, 4");
	...
	public final boolean registrationNecessary() {
	...
	private void enableButtonIfPossible(AWTEvent event) {
		if(okButton != null) {
			if(chckbxAllowbackup.isSelected()) {
				...
				else if(rdbtnRegister.isSelected() && chckbxAcceptLicense.isSelected() && txtUsername.getText().trim().length() > 2 && pwdPassword.getPassword().length > 5 && txtEmail.getText().trim().length() > 6 && pwdRetypepasswd.getPassword().length > 5) {	
					okButton.setEnabled(true);
				}
				else {
