...
<!--
    ...
    Description: transforms freeplane mm format to html, handles crossrefs and adds numbering. feel free to customize it while leaving the ancient authors
                    mentioned. thank you
    ChangeLog:
...
<xsl:key name="refid" match="node" use="@ID" />
    ...
    <xsl:template match="/">
        <xsl:text>#MindMapExport FreeplaneVersion:</xsl:text><xsl:value-of select="map/@version"/>
        <xsl:text>&#xA;</xsl:text>
