...
public class MenuUtilsNew {
	...
	public MenuUtilsNew(ActionAcceleratorManager acceleratorManager) {
	...
	public static DefaultMutableTreeNode createMenuEntryTree(final String menuRootKey) {
		Entry mainMenuEntry = mainMenuEntry(menuRootKey);
		final MenuUtilsNew menuUtils = instance();
		final DefaultMutableTreeNode treeRoot = new DefaultMutableTreeNode(null);
		menuUtils.addChildrenRecursively(treeRoot, mainMenuEntry.children());
		return treeRoot;
	}

	private static Entry mainMenuEntry(final String menuRootKey) {
		final MModeController modeController = (MModeController) Controller.getCurrentModeController();
		final IUserInputListenerFactory userInputListenerFactory = modeController.getUserInputListenerFactory();
		final Entry genericMenuStructure = userInputListenerFactory.getGenericMenuStructure();
		Entry mainMenuEntry = genericMenuStructure.getChild(menuRootKey);
		return mainMenuEntry;
	}

	private static MenuUtilsNew instance() {
		final MModeController modeController = (MModeController) Controller.getCurrentModeController();
		final IUserInputListenerFactory userInputListenerFactory = modeController.getUserInputListenerFactory();
		final ActionAcceleratorManager acceleratorManager = userInputListenerFactory.getAcceleratorManager();
		final MenuUtilsNew menuUtils = new MenuUtilsNew(acceleratorManager);
		return menuUtils;
	}
	...
	private DefaultMutableTreeNode menuNode2menuEntryNode(Entry menuItem) {
		...
		if (name != null) {
			...
			return new DefaultMutableTreeNode(new MenuEntry(name, text, iconKey,
			    accelerator, tooltip));
		}
	...
	public static DefaultMutableTreeNode createAcceleratebleMenuEntryTree(final String menuRootKey) {
		final DefaultMutableTreeNode menuEntryTreeNode = MenuUtilsNew.createMenuEntryTree(menuRootKey);
		final DefaultMutableTreeNode result = new DefaultMutableTreeNode(menuEntryTreeNode.getUserObject());
		instance().addAcceleratableChildrenRecursively(result, menuEntryTreeNode.children());
		return result;
	}

	// filters out non-acceleratable menu entries
	@SuppressWarnings("rawtypes")
	private void addAcceleratableChildrenRecursively(final DefaultMutableTreeNode target,
	...
	public static void insertAsNodeModelRecursively(final NodeModel nodeModel, final Enumeration children,
		...
		while (children.hasMoreElements()) {
			...
			final NodeModel newNodeModel = MenuUtilsNew.insertAsNodeModel(nodeModel, child, mapController);
			if (!child.isLeaf()) {
				MenuUtilsNew.insertAsNodeModelRecursively(newNodeModel, child.children(), mapController);
			}
		}
	}

	static private NodeModel insertAsNodeModel(final NodeModel nodeModel, final DefaultMutableTreeNode treeNode,
	                                           final MapController mapController) {
		...
		final String text = menuEntry.getKeyStroke() == null ? menuEntry.getLabel() : menuEntry.getLabel() + ": "
		        + MenuUtilsNew.formatKeyStroke(menuEntry.getKeyStroke());
