...
public class ExportHook extends ModeControllerHookAdapter {
	...
	protected File chooseFile(final String type, final String description,
		...
		final MModeController mindMapController = (MModeController) getController();
		...
		getController().getUrlManager().setLastCurrentDir(
		...
		if (chosenFile.exists()) {
			final String overwriteText = MessageFormat.format(getController()
	...
	protected void copyFromResource(final String prefix, final String fileName,
	                                final String destinationDirectory) {
		try {
			final URL resource = getResource(prefix + fileName);
			if (resource == null) {
