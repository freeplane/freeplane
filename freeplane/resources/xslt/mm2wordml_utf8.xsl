...
<o:DocumentProperties>
	<o:Title><xsl:value-of select="node/@TEXT"/></o:Title>
...
<xsl:otherwise>
    <wx:sub-section>
	  ...
	  <xsl:choose>
        ...
        <w:r>
          <w:t><xsl:call-template name="output-node-text-as-text"/></w:t>
        </w:r>
      </w:p>
...
<xsl:template match="node">
	<w:p>
		...
		<w:r>
			<w:t><xsl:call-template
					name="output-node-text-as-text"/></w:t>
		</w:r>
	</w:p>
	<xsl:call-template name="output-note-text-as-bodytext" />
	<xsl:apply-templates select="node" />
...
<xsl:template name="output-note-text-as-bodytext">
	...
	<w:p>
		...
		<w:r>
			<w:t><xsl:value-of
		select="string(richcontent[@TYPE='NOTE']/html/body)" /></w:t>
		</w:r>
	</w:p>
	</xsl:if>
</xsl:template>
